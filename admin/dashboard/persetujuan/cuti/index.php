...
if (isset($_GET['kode'])) {
    ...
    if ($_GET['req'] == 0) {
        if ($_SESSION['role'] == 2) {
            mysqli_query($conn, "UPDATE cuti SET approval=5, last_approval=(NOW()) WHERE id='$_GET[kode]'");
            $response = sendMessage(); // 1
            $return["allresponses"] = $response; // 2
            $return = json_encode($return); // 3 baris buat kirim notif
            echo "$alert";
        } else {
            mysqli_query($conn, "UPDATE cuti SET approval='$numprove', last_approval=(NOW()) WHERE id='$_GET[kode]'");
            $response = sendMessage(); // 1
            $return["allresponses"] = $response; // 2
            $return = json_encode($return); // 3 baris buat kirim notif
            echo "$alert";
...
if (isset($_POST['tolak'])) {
    ...
    if ($_GET['jnt'] == 'Cuti Tahunan') {
        $sisact  = $ct + $_GET['jt'];
        mysqli_query($conn, "UPDATE cuti, karyawan SET cuti.approval=10, cuti.last_approval=(NOW()), cuti.komentar='$_POST[komentar]' , karyawan.cuti_tahunan='$sisact' WHERE cuti.nrp=karyawan.nrp AND cuti.id='$_GET[kodee]' AND karyawan.nrp='$_GET[nrp]'");
        echo "<script>    
        Swal.fire({
        ...
        $sisacb  = $cb + $_GET['jt'];
        mysqli_query($conn, "UPDATE cuti, karyawan SET cuti.approval=10, cuti.last_approval=(NOW()), cuti.komentar='$_POST[komentar]' , karyawan.cuti_besar='$sisacb' WHERE cuti.nrp=karyawan.nrp AND cuti.id='$_GET[kodee]' AND karyawan.nrp='$_GET[nrp]'");
        echo "<script>    
        Swal.fire({
            title: 'Permintaan Cuti Berhasil Ditolak!',
            icon: 'error',
        ...
        mysqli_query($conn, "UPDATE cuti SET approval=10, last_approval=(NOW()), komentar='$_POST[komentar]' WHERE id='$_GET[kodee]'");
        echo "<script>    
            Swal.fire({
                title: 'Permintaan Cuti Berhasil Ditolak!',
                icon: 'error',
