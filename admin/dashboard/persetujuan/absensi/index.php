                        ...
                        <div class="col-12 card tumpul table-borderless">
                            <table id="datatablesSimple" class="table table-hover table-responsive border w-100 d-block d-md-table" style="color:black">
                                ...
                                <tbody class="table-light">
                                    ...
                                    $no = 1;
                                    error_reporting(1);
                                    $query = "SELECT * FROM absensi a INNER JOIN karyawan b ON a.nrp=b.nrp WHERE a.approval='$_SESSION[role]'";
...
function sendMessage()
{
    ...
    $content = array(
        "en" => "Terdapat Permintaan Koreksi Absensi"
    );
...
if (isset($_GET['kode'])) {
    ...
    if ($_SESSION['role'] == 0) {
        mysqli_query($conn, "UPDATE absensi SET approval='$numprove', last_approval=(NOW()) WHERE id='$_GET[kode]'");
        $response = sendMessage(); // 1
        $return["allresponses"] = $response; // 2
        $return = json_encode($return); // 3 baris buat kirim notif
        echo "<script>    
    Swal.fire({
        title: 'Permintaan Koreksi Absen Berhasil Disetujui!',
        icon: 'success',
    ...
    Swal.fire({
        ...
        mysqli_query($conn, "UPDATE absensi SET approval=5, last_approval=(NOW()) WHERE id='$_GET[kode]'");
        $response = sendMessage(); // 1
        $return["allresponses"] = $response; // 2
        $return = json_encode($return); // 3 baris buat kirim notif
        echo "<script>    
...
if (isset($_GET['kode1'])) {
    mysqli_query($conn, "UPDATE absensi SET approval=10, last_approval=(NOW()) WHERE id='$_GET[kode1]'");
    echo "<script>    
    Swal.fire({
        title: 'Permintaan Koreksi Absen Berhasil Ditolak!',
        icon: 'error',
        showCancelButton: false,
        allowOutsideClick : false,
        confirmButtonColor: '#3085d6',
