...
<head>
    ...
    <title>Lembur | Persetujuan</title>
                ...
                <div class="container-fluid">
                    ...
                    <div class="row">
                        <div class="col card-body tumpul bg-light-biru text-white" style="background-color: #de9ecc;">
                            ...
                            <span>Berikut adalah daftar Lembur Pekerjaan</span>
                        </div>
                    </div>
                        ...
                        <div class="col-12 card tumpul table-borderless">
                            <table id="datatablesSimple" class="table table-hover table-responsive border w-100 d-block d-md-table" style="color:black">
                                ...
                                <tbody class="table-light">
                                    ...
                                    $no = 1;
                                    error_reporting(1);
                                    $query = "SELECT * FROM lembur a INNER JOIN karyawan b ON a.nrp=b.nrp WHERE a.approval='$_SESSION[role]'";
                                    ...
                                    while ($tampil = mysqli_fetch_array($ambildata)) {
                                        ...
                                        if ($approval == 0) {
                                            ...
                                            $status = "<p style='color:green'> Lembur Anda Sudah Disetujui </p>";
                                        } else if ($approval == 10) {
                                            $status = "<p style='color:red'> Lembur Anda Ditolak </p>";
                                        }
                                    ?>
                                        ...
                                        <tr>
                                            ...
                                            <td><?php echo $tampil['jam_pulang'] ?></td>
...
function sendMessage()
{
    ...
    $content = array(
        "en" => "Terdapat Permintaan Claim Lembur"
    );
...
$alert = "<script>    
Swal.fire({
    title: 'Claim Lembur Berhasil Disetujui!',
    icon: 'success',
...
if (isset($_GET['kode'])) {
    ...
    if ($_GET['req'] == 0) {
        if ($_SESSION['role'] == 2) {
            mysqli_query($conn, "UPDATE lembur SET approval=5, last_approval=(NOW()) WHERE id='$_GET[kode]'");
            $response = sendMessage(); // 1
            $return["allresponses"] = $response; // 2
            $return = json_encode($return); // 3 baris buat kirim notif
            echo "$alert";
        } else {
            mysqli_query($conn, "UPDATE lembur SET approval='$numprove', last_approval=(NOW()) WHERE id='$_GET[kode]'");
            $response = sendMessage(); // 1
            $return["allresponses"] = $response; // 2
            $return = json_encode($return); // 3 baris buat kirim notif
            echo "$alert";
...
if (isset($_GET['kodee'])) {
    mysqli_query($conn, "UPDATE lembur SET approval=10, last_approval=(NOW()) WHERE id='$_GET[kodee]'");
    echo "<script>    
        Swal.fire({
            title: 'Claim lembur Berhasil Ditolak!',
            icon: 'error',
            showCancelButton: false,
            allowOutsideClick : false,
            confirmButtonColor: '#3085d6',
